...
严禁把 `WECHAT_APP_SECRET` 放进 Wrangler vars、Worker secrets、`.dev.vars`、D1、日志或聊天。
AppID 可以提交，其他值只能通过受保护文件或交互式终端传递。

## 检查与同步

```bash
# 只读：检查文件权限、AppID 漂移、Worker binding 类型、回调签名和出口鉴权
...
同步脚本不会读取或上传 `WECHAT_APP_SECRET`，只验证它在 cloudclone 中存在且格式正确。
临时密钥文件位于权限为 700 的 `/tmp` 目录，退出时逐个清理，任何密钥都不会打印。
